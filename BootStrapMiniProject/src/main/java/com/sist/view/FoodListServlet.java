...
public class FoodListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(FoodVO vo:list) {
			...
			out.println("<a href=FoodDetailBeforeServlet?mode=3&fno="+vo.getFno()+"><img src="+vo.getPoster()+" class=img-rounded style=\"width:200px;height:200px\"></a>");
			out.println("</td>");
		...
		out.println("<div class=row>");
			out.println("<h3>최근 본 맛집</h3>");
			out.println("<a href=\"FoodCookieDeleteServlet?mode=2&cno="+cno+"\" class=\"btn btn-xs btn-danger\">전체 삭제</a>");
			out.println("<hr>");
		// cookie 처리
		// 1. 쿠키 읽어오기
		...
		if(cookies!=null)
		{
			for(int i=cookies.length-1;i>=0;i--) // 최신에 본거 먼저 읽어옴
			{
				if(cookies[i].getName().startsWith("food_"))
				{
					...
					// 테이블 컬럼에 저장시에는 한개의 값만 저장 → 관리가 쉬움
					vo.setPoster(temp);
					cList.add(vo);
				}
			}
		...
		if(cList!=null && cList.size()>0)
		{
			for(FoodVO vo:cList)
			{
				...
				out.println("<div class=\"col-md-3\">");
					...
					out.println("<img src=\""+vo.getPoster()+"\" style=\"width:200px; height:200px\"></a>");
