...
public class FoodDAO {
	...
	public List<FoodVO> foodCategoryListData(int cno){
		...
		try {
			...
			String sql = "SELECT fno, cno, poster, name, score, address, NVL(foodReplyData(fno), ' '), NVL(foodReplyName(fno), ' ') as rname "
	...
	public FoodVO foodDetailData(int fno) {
		...
		try {
			...
			String sql = "UPDATE food_house SET hit=hit+1 WHERE fno=?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, fno);
			ps.executeUpdate();
			
			sql = "SELECT fno, cno, name, score, address, phone, type, time, parking, price, menu, poster FROM food_house WHERE fno=?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, fno);
			ResultSet rs = ps.executeQuery();
	...
	public List<FoodVO> foodTop7(){
		...
		try {
			...
			String sql = "SELECT fno, name, hit, rownum "
					+ "FROM (SELECT fno, name, hit FROM food_house ORDER BY hit DESC) "
					+ "WHERE rownum<=7";
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
